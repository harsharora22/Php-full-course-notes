...
$offset = ($page - 1) * $limit;
$Ssql = "SELECT post.post_id,post.title,post.description,post.post_date,category.category_name,user.username,post.category,post.post_img,post.author FROM post
...
if (mysqli_num_rows($result) > 0) {
    ...
    echo "<h2>No news posted yet</h2>";
}

if (mysqli_num_rows($result1) > 0) {
    $totalRecords = $row1['post'];

    $totalPages = ceil($totalRecords / $limit);
    echo "<ul class='pagination admin-pagination'>";
    if ($page > 1) {
        echo "<li><a href='category.php?id={$id} & page=" . ($page - 1) . "'> &#8592 </a></li>";
    }
    ...
    if ($totalPages > $page) {
        echo "<li><a href='category.php?id={$id} & page=" . ($page + 1) . "'>  &#8594 </a></li>";
    }
    echo "</ul>";
}
?>
