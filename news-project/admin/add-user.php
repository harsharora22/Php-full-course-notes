...
if (isset($_POST['save'])) {
   ...
   $Ssql = "SELECT username FROM user WHERE username = '{$username}'";
    $result = mysqli_query($conn, $Ssql) or die("Query Failed");
    if (mysqli_num_rows($result) > 0) {
        echo "<p style='color:red;text-align:center;margin:10px 0;'>UserName already exists</p>";
    } else {
        $Isql = "INSERT INTO user (first_name,last_name,username,password,role) VALUES ('{$fname}','{$lname}','{$username}','{$password}','{$role}')";
